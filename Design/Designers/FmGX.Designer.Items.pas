...
uses
  ...
  FMX.TreeView, FMX.ActnList, FMX.ListBox, FMX.Controls.Presentation, FMX.ImgList, FMX.Menus, FmGX.Toolbar, FmGX.BitBtn,
  FmGX.Items, FmGX.LinkedLabel;
...
type
  ...
  private
    ...
    [Weak] FItemsList: TList<TfgxItemInformation>;
    ...
    function AddTreeNode(const ARootNode: TFmxObject; const AItem: TFmxObject; const AItemInformation: TfgxItemInformation; const AIndex: Integer = -1): TTreeViewItemWithData;
  ...
  private
    ...
    FItemInformation: TfgxItemInformation;
  public
    property Item: TFmxObject read FItemComponent write FItemComponent;
    property Information: TfgxItemInformation read FItemInformation write FItemInformation;
  end;
...
uses
  System.TypInfo, System.SysUtils, System.Math, System.Rtti, System.Generics.Defaults, FMX.Platform, FmGX.Asserts;
...
begin
  TfgxAssert.IsNotNil(FItemsList);
  TfgxAssert.IsNotNil(FComponent);
  TfgxAssert.IsNotNil(Designer);
  TfgxAssert.IsNotNil(SelectedNode);
  TfgxAssert.InRange(ComboBoxItemsInfo.ItemIndex, 0, ComboBoxItemsInfo.Items.Count - 1);
  TfgxAssert.InRange(ComboBoxItemsInfo.ItemIndex, 0, FItemsList.Count - 1);
...
var
  ...
  Node: TTreeViewItem;
  NewIndex: Integer;
  ItemInformation: TfgxItemInformation;
begin
  TfgxAssert.IsNotNil(FItemsList);
  TfgxAssert.IsNotNil(FComponent);
  TfgxAssert.IsNotNil(Designer);
...
begin
  ...
  begin
    ...
    begin
      ...
      TfgxAssert.IsNotNil(SourceNode.Item);
...
function TfgFormItemsDesigner.AddTreeNode(const ARootNode: TFmxObject; const AItem: TFmxObject;
  const AItemInformation: TfgxItemInformation; const AIndex: Integer = -1): TTreeViewItemWithData;
var
  Node: TTreeViewItemWithData;
begin
  TfgxAssert.IsNotNil(AItem);
  TfgxAssert.IsNotNil(ARootNode);
  TfgxAssert.IsClass(AItem, TControl);
...
begin
  ...
  begin
    FItemsList := TfgxItemsManager.GetListByComponentClass(TFmxObjectClass(Component.GetObject.ClassType));
    FillClassItemsInfo;
    UpdateActions;
  end
  else
...
procedure TfgFormItemsDesigner.RebuildTree;
  ...
  function FindItemInformation(const AItem: TFmxObject): TfgxItemInformation;
  var
    ItemInf: TfgxItemInformation;
  begin
  ...
  var
    ...
    Information: TfgxItemInformation;
  begin
...
var
  Node: TTreeViewItemWithData;
begin
  TfgxAssert.IsNotNil(ANodeStyleObject);
  TfgxAssert.IsNotNil(Designer);
  TfgxAssert.IsClass(ANodeStyleObject, TFmxObject);
...
var
  Item: TFmxObject;
begin
  TfgxAssert.IsNotNil(ANode);
  TfgxAssert.IsNotNil(ANode.Item);
  TfgxAssert.InRange(AIndex, 0, ANode.Parent.ChildrenCount - 1);
  TfgxAssert.IsNotNil(TreeView);
  TfgxAssert.IsNotNil(SelectedNode);
  TfgxAssert.IsNotNil(Component);
...
begin
  TfgxAssert.IsNotNil(ANode);
  TfgxAssert.IsNotNil(ANode.Item);
  TfgxAssert.IsClass(ANode.Item, TControl);
