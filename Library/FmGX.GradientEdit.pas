...
type
  ...
  TfgxGradientEdit = class (TfgxCustomGradientEdit)
  published
...
uses
  System.Math.Vectors, System.SysUtils, System.Math, FMX.Colors, FMX.Types, FmGX.Graphics, FmGX.Helpers, FmGX.Asserts;

{ TfgCustomGradientEdit }

constructor TfgxCustomGradientEdit.Create(AOwner: TComponent);
begin
...
destructor TfgxCustomGradientEdit.Destroy;
begin
...
procedure TfgxCustomGradientEdit.DoGradientChanged(Sender: TObject);
begin
  Repaint;
end;

procedure TfgxCustomGradientEdit.DoPointRemoved(AGradientPoint: TGradientPoint);
begin
  TfgxAssert.IsNotNil(AGradientPoint);

  if Assigned(FOnPointRemoved) then
    FOnPointRemoved(Self, AGradientPoint);
end;

procedure TfgxCustomGradientEdit.DoPointAdded(AGradientPoint: TGradientPoint);
begin
  TfgxAssert.IsNotNil(AGradientPoint);
...
procedure TfgxCustomGradientEdit.DoPointDblClick(const AGradientPoint: TGradientPoint);
begin
  TfgxAssert.IsNotNil(AGradientPoint);
...
procedure TfgxCustomGradientEdit.DoPointClick(const AGradientPoint: TGradientPoint);
begin
  TfgxAssert.IsNotNil(AGradientPoint);
...
procedure TfgxCustomGradientEdit.DrawBackground;
var
  GradientRect: TRectF;
begin
...
procedure TfgxCustomGradientEdit.DrawGradient;
var
  GradientRect: TRectF;
begin
  GradientRect := GetGradientFieldSize;
...
procedure TfgxCustomGradientEdit.DrawPoint(const AIndex: Integer; const ASelected: Boolean = False);
var
  Outline: array of TPointF;
  SelectedTriangle: TPolygon;
  FillRect: TRectF;
begin
  TfgxAssert.InRange(AIndex, 0, FGradient.Points.Count - 1);
...
function TfgxCustomGradientEdit.FindPointAtPoint(const APoint: TPointF; var AIndex: Integer): Boolean;
var
  I: Integer;
  Found: Boolean;
  Offset: Extended;
...
function TfgxCustomGradientEdit.GetDefaultSize: TSizeF;
begin
  Result := TSizeF.Create(100, 35);
end;

function TfgxCustomGradientEdit.GetGradientFieldSize: TRectF;
begin
  Result := TRectF.Create(PickerSize / 2, 0, Width - PickerSize / 2, Height - 1.25 * PickerSize);
end;

function TfgxCustomGradientEdit.IsBorderRadiusStored: Boolean;
begin
  Result := not SameValue(BorderRadius, DEFAULT_BORDER_RADIUS, Single.Epsilon);
end;

function TfgxCustomGradientEdit.IsPickerSizeStored: Boolean;
begin
  Result := not SameValue(PickerSize, DEFAULT_PICKER_SIZE, Single.Epsilon);
end;

procedure TfgxCustomGradientEdit.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Single);
var
  PointIndex: Integer;
...
procedure TfgxCustomGradientEdit.MouseMove(Shift: TShiftState; X, Y: Single);
var
  NewOffset: Extended;
...
procedure TfgxCustomGradientEdit.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Single);
begin
...
procedure TfgxCustomGradientEdit.DoChanged;
begin
  if Assigned(FOnChanged) then
    FOnChanged(Self);
end;

procedure TfgxCustomGradientEdit.DoChangeTracking;
begin
...
procedure TfgxCustomGradientEdit.Paint;
...
function TfgxCustomGradientEdit.Selected: Boolean;
begin
  Result := FSelectedPoint <> nil;
end;

procedure TfgxCustomGradientEdit.SetBorderColor(const Value: TAlphaColor);
begin
...
procedure TfgxCustomGradientEdit.SetBorderRadius(const Value: Single);
begin
...
procedure TfgxCustomGradientEdit.SetGradient(const Value: TGradient);
begin
  Assert(Value <> nil);
  FGradient.Assign(Value);
end;

procedure TfgxCustomGradientEdit.SetPickerSize(const Value: Single);
begin
...
procedure TfgxCustomGradientEdit.UpdateGradientPointsOrder(APoint: TGradientPoint);
var
  I: Integer;
  Found: Boolean;
  PointsCount: Integer;
...
begin
  TfgxAssert.IsNotNil(FGradient);
  TfgxAssert.IsNotNil(APoint);
...
initialization
  RegisterFmxClasses([TfgxCustomGradientEdit, TfgxGradientEdit]);
end.
