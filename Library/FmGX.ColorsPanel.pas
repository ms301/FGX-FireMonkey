...
uses
  System.Types, System.Classes, System.UITypes, FMX.Controls, FMX.Graphics, FMX.Types, FmGX.Colors.Presets, FmGX.Types,
  FmGX.Consts;
...
type
  ...
  TfgxOnGetColor = procedure(Sender: TObject; const Column, Row: Integer; var Color: TAlphaColor) of object;
  TfgxOnColorSelected = procedure(Sender: TObject; const AColor: TAlphaColor) of object;
  TfgxOnPaintCell = procedure(Sender: TObject; Canvas: TCanvas; const Column, Row: Integer; const Frame: TRectF;
    const AColor: TAlphaColor; Corners: TCorners; var Done: Boolean) of object;

  TfgxColorsPresetKind = (WebSafe, X11, Custom);

  TfgxCustomColorsPanel = class(TControl)
  public const
    DefaultCellSize = 18;
    MinCellSize = 5;
  private
    FCellSize: TfgxSingleSize;
    FBorderRadius: Single;
    FStrokeBrush: TStrokeBrush;
    FColorsPreset: TfgColorsPreset;
    FPresetKind: TfgxColorsPresetKind;
    FOnGetColor: TfgxOnGetColor;
    FOnColorSelected: TfgxOnColorSelected;
    FOnPaintCell: TfgxOnPaintCell;
    function IsBorderRadiusStored: Boolean;
    function IsCellSizeStored: Boolean;
    procedure SetColorCellSize(const Value: TfgxSingleSize);
    ...
    procedure SetPresetKind(const Value: TfgxColorsPresetKind);
  protected
    { Events }
    ...
    procedure DoPaintCell(const AColumn, ARow: Integer; const AFrame: TRectF; const AColor: TAlphaColor;
      ACorners: TCorners; var ADone: Boolean); virtual;
  ...
  public
    ...
    property PresetKind: TfgxColorsPresetKind read FPresetKind write SetPresetKind default TfgxColorsPresetKind.WebSafe;
    property Stroke: TStrokeBrush read FStrokeBrush write SetBorderColor;
    property CellSize: TfgxSingleSize read FCellSize write SetColorCellSize stored IsCellSizeStored;
    property OnGetColor: TfgxOnGetColor read FOnGetColor write FOnGetColor;
    property OnColorSelected: TfgxOnColorSelected read FOnColorSelected write FOnColorSelected;
    property OnPaintCell: TfgxOnPaintCell read FOnPaintCell write FOnPaintCell;
  end;

  { TfgColorsPanel }

  [ComponentPlatformsAttribute(fgAllPlatform)]
  TfgxColorsPanel = class(TfgxCustomColorsPanel)
  published
    property Stroke;
...
uses
  System.Math, System.SysUtils, System.UIConsts, System.TypInfo, FmGX.Graphics, FmGX.Asserts;

{ TfgCustomColorsPanel }

function TfgxCustomColorsPanel.ColumnsCount: Integer;
begin
  TfgxAssert.IsNotNil(CellSize);
...
constructor TfgxCustomColorsPanel.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FCellSize := TfgxSingleSize.Create(Self, DefaultCellSize, DefaultCellSize, DoCellSizeChanged);
  FBorderRadius := 0;
...
destructor TfgxCustomColorsPanel.Destroy;
begin
...
procedure TfgxCustomColorsPanel.DoBorderStrokeChanged(Sender: TObject);
begin
  Repaint;
end;

procedure TfgxCustomColorsPanel.DoCellSizeChanged(Sender: TObject);
begin
  Repaint;
end;

procedure TfgxCustomColorsPanel.DoColorSelected(const AColor: TAlphaColor);
begin
  if Assigned(FOnColorSelected) then
    FOnColorSelected(Self, AColor);
end;

procedure TfgxCustomColorsPanel.DoGetColor(const AColumn, ARow: Integer; var AColor: TAlphaColor);
begin
  if Assigned(FOnGetColor) then
    FOnGetColor(Self, AColumn, ARow, AColor);
end;

procedure TfgxCustomColorsPanel.DoPaintCell(const AColumn, ARow: Integer; const AFrame: TRectF;
  const AColor: TAlphaColor; ACorners: TCorners; var ADone: Boolean);
begin
  if Assigned(FOnPaintCell) then
    FOnPaintCell(Self, Canvas, AColumn, ARow, AFrame, AColor, ACorners, ADone);
end;

procedure TfgxCustomColorsPanel.DrawCell(const AColumn, ARow: Integer; const AFrame: TRectF; const AColor: TAlphaColor);
...
var
  ...
  Done: Boolean;
begin
  TfgxAssert.IsNotNil(Canvas);
  TfgxAssert.InRange(AColumn, 1, ColumnsCount);
  TfgxAssert.InRange(ARow, 1, RowsCount);
...
function TfgxCustomColorsPanel.GetDefaultSize: TSizeF;
begin
  Result := TSizeF.Create(85, 34);
end;

function TfgxCustomColorsPanel.GetCellFrame(const Column, Row: Integer): TRectF;
var
  Left: Single;
  Top: Single;
...
begin
  TfgxAssert.IsNotNil(CellSize);
  TfgxAssert.InRange(Column, 1, ColumnsCount);
  TfgxAssert.InRange(Row, 1, RowsCount);
...
function TfgxCustomColorsPanel.GetBorderFrame: TRectF;
var
  HalfThickness: Single;
begin
  HalfThickness := FStrokeBrush.Thickness / 2;
...
function TfgxCustomColorsPanel.GetColor(const AColumn, ARow: Integer): TAlphaColor;
var
  ColorIndex: Integer;
...
function TfgxCustomColorsPanel.IsBorderRadiusStored: Boolean;
begin
  Result := not SameValue(BorderRadius, 0, Single.Epsilon);
end;

function TfgxCustomColorsPanel.IsCellSizeStored: Boolean;
begin
  Result := not SameValue(FCellSize.Width, DefaultCellSize, Single.Epsilon) or
    not SameValue(FCellSize.Height, DefaultCellSize, Single.Epsilon);
end;

procedure TfgxCustomColorsPanel.MouseClick(Button: TMouseButton; Shift: TShiftState; X, Y: Single);
var
...
procedure TfgxCustomColorsPanel.Paint;
var
  Column: Integer;
  Row: Integer;
  Color: TAlphaColor;
...
function TfgxCustomColorsPanel.RowsCount: Integer;
begin
  Result := Floor(Height / (CellSize.Height - 1));
end;

procedure TfgxCustomColorsPanel.SetBorderColor(const Value: TStrokeBrush);
begin
...
procedure TfgxCustomColorsPanel.SetBorderRadius(const Value: Single);
begin
...
procedure TfgxCustomColorsPanel.SetColorCellSize(const Value: TfgxSingleSize);
begin
...
procedure TfgxCustomColorsPanel.SetPresetKind(const Value: TfgxColorsPresetKind);
begin
...
procedure TfgxCustomColorsPanel.SetColorsPreset(const Value: TfgColorsPreset);
begin
  FColorsPreset := Value;
  Repaint;
end;
...
RegisterFmxClasses([TfgxCustomColorsPanel, TfgxColorsPanel]);
