...
unit FmGX.FlipView.Presentation;

interface

uses
  System.Classes, System.Types, FMX.Presentation.Style, FMX.Graphics,  FMX.Types, FMX.Controls.Model,
  FMX.Presentation.Messages, FMX.Objects, FMX.Controls, FmGX.FlipView, FmGX.FlipView.Types;
...
{ TStyledFlipViewBasePresentation }
  ...
  protected
    ...
    procedure PMGoToImage(var Message: TDispatchMessageWithValue<TfgxShowImageInfo>); message TfgxFlipViewMessages.PM_GO_TO_IMAGE;
    { Messages From Model}
    procedure MMItemIndexChanged(var AMessage: TDispatchMessageWithValue<Integer>); message TfgxFlipViewMessages.MM_ITEM_INDEX_CHANGED;
    procedure MMShowNavigationButtons(var AMessage: TDispatchMessageWithValue<Boolean>); message TfgxFlipViewMessages.MM_SHOW_NAVIGATION_BUTTONS_CHANGED;
  protected
  ...
  public
    ...
    procedure ShowNextImage(const ANewItemIndex: Integer; const ADirection: TfgxDirection; const AAnimate: Boolean); virtual;
...
uses
  System.SysUtils, System.Math, System.UITypes, FMX.Styles, FmGX.Asserts;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Vector.X < 0 then
        ShowNextImage(IfThen(Model.IsLastImage, 0, Model.ItemIndex + 1), TfgxDirection.Forward, True)
      else
        ShowNextImage(IfThen(Model.IsFirstImage, Model.ImagesCount - 1, Model.ItemIndex - 1), TfgxDirection.Backward, True);
    end;
  end;
end;
...
function TfgStyledFlipViewBasePresentation.GetModel: TfgxFlipViewModel;
begin
...
begin
  TfgxAssert.IsClass(PresentedControl, TfgxCustomFlipView);

  if not FIsGestureStarted and Assigned(Model.OnImageClick) and (PresentedControl is TfgxCustomFlipView) and (Model.ItemIndex <> -1) then
    Model.OnImageClick(PresentedControl, TfgxCustomFlipView(PresentedControl), Model.ItemIndex);
...
procedure TfgStyledFlipViewBasePresentation.PMGoToImage(var Message: TDispatchMessageWithValue<TfgxShowImageInfo>);
begin
  ShowNextImage(Message.Value.NewItemIndex, Message.Value.Direction, Message.Value.Animate);
end;

procedure TfgStyledFlipViewBasePresentation.ShowNextImage(const ANewItemIndex: Integer; const ADirection: TfgxDirection;
